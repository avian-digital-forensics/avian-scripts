...
# To add default script.settings, create a default settings file.
...
script.dialog_append_save_file_chooser('main_tab', 'output_path', 'Output path', 'Comma Seperated script.settings', 'csv',
...
script.dialog_validate_before_closing do |script.settings|
    # Make sure primary address is not empty.
    if script.settings['primary_address'].strip.empty?
        CommonDialogs.show_warning('Please provide a non-empty primary address.', 'No Primary Address')
        next false
    end
    # Make sure path is not empty.
    if script.settings['output_path'].strip.empty?
    ...
    if script.settings['delimiter'] == 'custom' && script.settings['custom_delimiter'].strip.empty?
...
script.run do |progress_dialog|
  ...
  # What delimiter is used between script.settings on the same line.
  delimiter = script.settings['delimiter']
  if delimiter == 'custom'
      delimiter = script.settings['custom_delimiter']
  end
  
  ConnectedAddresses::connected_addresses(current_case, progress_dialog, timer, file_path, delimiter)
end
