module LoadTimer
    ...
    def run_init(wss_global)
        ...
        wss_global.vars[:load_timer_file_path] = File.join(wss_global.case_data_path, 'load_times_' + id + '.txt')
        File.open(wss_global.vars[:load_timer_file_path], 'a') { |file| file.puts('start_time:' + wss_global.vars[:load_timer_start_time].to_s) }
    end
    ...
    def run(wss_global, worker_item)
        ...
        File.open(wss_global.vars[:load_timer_file_path], 'a') { |file| file.puts(worker_item.item_guid + ':' + current_time_nano.to_s) }
    ...
    def run_close(wss_global)
        ...
        File.open(wss_global.vars[:load_timer_file_path], 'a') { |file| file.puts('end_time:' + Utils::nano_now.to_s) }
    end
end
